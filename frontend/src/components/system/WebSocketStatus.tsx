  ...
  return (
    <div className="bg-white dark:bg-[#131D2E] border border-slate-200 dark:border-slate-700/50 rounded-xl p-5">
      <div className="flex items-center gap-2 mb-4">
        {connected ? <Wifi className="h-4 w-4 text-emerald-500 dark:text-emerald-400" /> : <WifiOff className="h-4 w-4 text-red-500 dark:text-red-400" />}
        <h3 className="text-sm font-semibold text-slate-900 dark:text-white">WebSocket</h3>
      </div>
      <div className="space-y-3">
        ...
        {socketId && (
          <div className={rowCls}>
            <span className="text-sm text-slate-600 dark:text-slate-400">Socket ID</span>
            <code className="text-xs text-slate-700 dark:text-slate-300 bg-slate-100 dark:bg-slate-700 px-2 py-0.5 rounded font-mono truncate max-w-[140px]">{socketId}</code>
          </div>
        )}
        <div className={rowCls}>
