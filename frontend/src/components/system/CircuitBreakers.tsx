    ...
    <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
      {cbs.map((cb, i) => (
        <motion.div key={cb.service} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}
          ...
          className="bg-white dark:bg-[#131D2E] border border-slate-200 dark:border-slate-700/50 rounded-xl p-4 hover:border-slate-300 dark:hover:border-slate-600 transition-colors">
          <div className="flex items-center justify-between mb-2">
            <div className="flex items-center gap-2">
              <span className={`w-2 h-2 rounded-full shrink-0 ${cbDot(cb.state)}`} />
              <span className="text-xs font-semibold text-slate-900 dark:text-white capitalize">{cb.service.replace(/_/g, ' ')}</span>
            </div>
            <Badge variant={cbBadge(cb.state)}>{cb.state}</Badge>
          </div>
          {cb.failures > 0 && <p className="text-xs text-slate-500">{cb.failures} failure{cb.failures !== 1 ? 's' : ''}</p>}
        </motion.div>
      ))}
    </div>
  )
