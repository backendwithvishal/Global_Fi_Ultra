...
const D: Record<BadgeVariant,string> = {
  ...
  amber: 'bg-amber-400', slate: 'bg-slate-400', purple: 'bg-purple-400', cyan: 'bg-cyan-400',
}
...
export function Badge({ variant = 'slate', children, className = '', dot }: BadgeProps) {
  return (
    <span className={`inline-flex items-center gap-1 rounded-full border px-2 py-0.5 text-xs font-medium ${V[variant]} ${className}`}>
      {dot && <span className={`w-1.5 h-1.5 rounded-full ${D[variant]}`} />}
      {children}
    </span>
  )
}
