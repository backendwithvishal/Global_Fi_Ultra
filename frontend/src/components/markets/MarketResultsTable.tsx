  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-2 flex-wrap">
        <Badge variant={data.status === 'success' ? 'green' : data.status === 'partial' ? 'amber' : 'red'}>{data.status}</Badge>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        {stock && (
          <div className={`${cardCls} p-5`}>
            <div className="flex items-center justify-between mb-4">
              <div>
                <h3 className="text-base font-bold text-slate-900 dark:text-white">{stock.symbol}</h3>
                {stock.name && <p className="text-xs text-slate-500">{stock.name}</p>}
              </div>
              {stock.exchange && <Badge variant="blue">{stock.exchange}</Badge>}
            </div>
            <div className="flex items-end gap-3 mb-4">
              <span className="text-4xl font-black text-slate-900 dark:text-white tabular-nums">{fmtUSD(stock.price)}</span>
              {stock.changePercent !== undefined && (
                <span className={`flex items-center gap-0.5 text-sm font-semibold mb-1 ${(stock.changePercent ?? 0) >= 0 ? 'text-emerald-600 dark:text-emerald-400' : 'text-red-600 dark:text-red-400'}`}>
            ...
            <div className="grid grid-cols-3 gap-2">
              {[
                { label: 'Open', v: fmtUSD(stock.open) }, { label: 'High', v: fmtUSD(stock.high) },
                { label: 'Low', v: fmtUSD(stock.low) }, { label: 'Prev Close', v: fmtUSD(stock.previousClose) },
                { label: 'Volume', v: fmtCompact(stock.volume) }, { label: 'P/E Ratio', v: stock.pe?.toFixed(2) ?? '—' },
              ].map(s => (
                <div key={s.label} className="bg-slate-50 dark:bg-slate-800/40 rounded-lg p-2.5">
                  <p className="text-[10px] text-slate-500 uppercase tracking-wide">{s.label}</p>
                  <p className="text-sm font-semibold text-slate-900 dark:text-white tabular-nums mt-0.5">{s.v}</p>
                </div>
              ))}
            </div>
          </div>
        ...
        {cryptos.length > 0 && (
          <div className={`${cardCls} overflow-hidden`}>
            <div className="px-5 py-3.5 border-b border-slate-200 dark:border-slate-700/50">
              <h3 className="text-sm font-semibold text-slate-900 dark:text-white">Crypto Prices</h3>
            </div>
            <table className="w-full text-sm">
              ...
              <tbody>
                {cryptos.map((c: any) => {
                  ...
                  return (
                    <tr key={c.id ?? c.symbol} className="border-b border-slate-100 dark:border-slate-700/30 hover:bg-slate-50 dark:hover:bg-slate-800/20">
                      ...
                      <td className="px-4 py-3 text-right font-semibold text-slate-900 dark:text-white tabular-nums">{fmtUSD(c.current_price)}</td>
                      <td className="px-4 py-3 text-right">
                        <span className={`inline-flex items-center gap-0.5 text-xs font-medium ${pos ? 'text-emerald-600 dark:text-emerald-400' : 'text-red-600 dark:text-red-400'}`}>
                          {pos ? <ArrowUpRight className="h-3 w-3" /> : <ArrowDownRight className="h-3 w-3" />}
                          {Math.abs(c.price_change_percentage_24h ?? 0).toFixed(2)}%
                        </span>
                      </td>
                      <td className="px-5 py-3 text-right text-slate-500 text-xs hidden sm:table-cell tabular-nums">{fmtCompact(c.market_cap)}</td>
                    </tr>
                  )
                })}
              </tbody>
      ...
      {forex && (
        <div className={`${cardCls} p-5`}>
          <div className="flex items-center gap-2 mb-4">
            <h3 className="text-sm font-semibold text-slate-900 dark:text-white">Forex Rates</h3>
          ...
          <div className="grid grid-cols-4 sm:grid-cols-6 lg:grid-cols-10 gap-2">
            {Object.entries(forex.rates ?? {}).slice(0, 20).map(([cur, rate]) => (
              <div key={cur} className="flex flex-col items-center p-2.5 rounded-lg bg-slate-50 dark:bg-slate-800/40 hover:bg-slate-100 dark:hover:bg-slate-800/70 transition-colors">
                <span className="text-xs font-bold text-slate-900 dark:text-white">{cur}</span>
