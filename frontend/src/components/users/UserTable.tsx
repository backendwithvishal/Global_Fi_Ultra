  ...
  if (users.length === 0) return (
    <div className="flex flex-col items-center justify-center py-16 text-center bg-white dark:bg-[#131D2E] border border-slate-200 dark:border-slate-700/50 rounded-xl">
      <div className="p-4 rounded-2xl bg-slate-100 dark:bg-slate-800/60 mb-4">
        <UserX className="h-8 w-8 text-slate-400 dark:text-slate-600" />
      </div>
          ...
          <AnimatePresence>
            {users.map((u, idx) => (
              <motion.tr key={u._id} initial={{ opacity: 0, y: 4 }} animate={{ opacity: 1, y: 0 }}
                ...
                className="border-b border-slate-100 dark:border-slate-700/30 hover:bg-slate-50 dark:hover:bg-slate-800/20 transition-colors group">
                <td className="px-5 py-3">
                  <div className="flex items-center gap-3">
                    ...
                    <span className="font-semibold text-slate-900 dark:text-white">{u.firstName} {u.lastName}</span>
                  </div>
                </td>
                <td className="px-4 py-3 text-slate-600 dark:text-slate-400 text-xs truncate max-w-[160px]">{u.email}</td>
                <td className="px-4 py-3 text-slate-600 dark:text-slate-400 text-xs hidden sm:table-cell">{u.preferences?.defaultCurrency ?? 'USD'}</td>
                <td className="px-4 py-3"><Badge variant={u.isActive ? 'green' : 'red'} dot>{u.isActive ? 'Active' : 'Inactive'}</Badge></td>
