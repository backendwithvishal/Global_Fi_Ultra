      ...
      <div className="space-y-4">
        <div className={cardCls}>
          ...
          {currentUser ? (
            <div className="space-y-4">
              <div className="flex items-center gap-4 p-4 rounded-xl bg-slate-50 dark:bg-slate-800/40">
                ...
                <div className="flex-1 min-w-0">
                  <p className="font-semibold text-slate-900 dark:text-white truncate">{currentUser.firstName} {currentUser.lastName}</p>
              ...
              <div className="grid grid-cols-2 gap-3 text-sm">
                {[
                  ...
                  <div key={r.label} className="p-3 rounded-xl bg-slate-50 dark:bg-slate-800/40">
                    ...
                    <p className="font-medium text-slate-900 dark:text-white mt-0.5">{r.value}</p>
                  </div>
                ))}
              </div>
            ...
            <div className="text-center py-6">
              <p className="text-sm text-slate-500 mb-4">Sign in to manage your account.</p>
        ...
        <div className={cardCls}>
          <div className="flex items-center gap-2 mb-4">
            ...
            <h3 className="text-sm font-semibold text-slate-900 dark:text-white">API Configuration</h3>
          </div>
          <div className="space-y-1">
            {[
              { label: 'API Base URL', value: '/api/v1' }, { label: 'WebSocket', value: 'ws://localhost:4000' },
              { label: 'API Version',  value: '1.0.0' },   { label: 'Global Limit', value: '100 req / 15 min' },
              { label: 'AI Limit',     value: '10 req / 1 min' },
            ].map(r => (
              <div key={r.label} className="flex items-center justify-between py-2.5 border-b border-slate-100 dark:border-slate-700/40 last:border-0">
                <span className="text-sm text-slate-600 dark:text-slate-400">{r.label}</span>
                <code className="text-xs bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 px-2 py-1 rounded-lg font-mono text-slate-700 dark:text-slate-300">{r.value}</code>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className={cardCls}>
          <div className="flex items-center gap-2 mb-4">
            ...
            <h3 className="text-sm font-semibold text-slate-900 dark:text-white">External Data Sources</h3>
          </div>
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
            {[
              { name: 'Alpha Vantage', type: 'Stocks', v: 'blue' }, { name: 'CoinGecko', type: 'Crypto', v: 'amber' },
              { name: 'ExchangeRate',  type: 'Forex',  v: 'purple' }, { name: 'NewsAPI',  type: 'News',   v: 'cyan' },
              { name: 'FRED',    type: 'Economic',    v: 'green' }, { name: 'Finnhub', type: 'Market News', v: 'blue' },
            ].map(a => (
              <div key={a.name} className="flex flex-col gap-1.5 p-3 rounded-xl border border-slate-200 dark:border-slate-700/60 bg-slate-50 dark:bg-slate-800/30 hover:bg-slate-100 dark:hover:bg-slate-800/60 transition-colors">
