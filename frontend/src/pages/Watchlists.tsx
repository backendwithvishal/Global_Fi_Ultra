...
// Level 4 modal
const modalCls = 'relative w-full max-w-md bg-white dark:bg-[#1A2540] border border-slate-200 dark:border-[var(--border-md)] rounded-2xl p-6 shadow-2xl z-10'
...
export function Watchlists() {
  ...
  const cf = useForm<CreateForm>({
    ...
    defaultValues: { name: '', description: '', isPublic: false, userId: currentUser?._id ?? '' },
  })
  ...
  const handleDelete = async (id: string, name: string) => {
    ...
    toast.success('Deleted', `"${name}" removed.`); setDeleting(null)
  }
  ...
  const handleAddAsset = async (data: AddAssetForm) => {
    ...
    try {
      ...
      setWatchlists(p => p.map(w => w._id === selectedWL._id
        ? { ...w, assets: [...w.assets, { symbol: data.symbol.toUpperCase(), addedAt: new Date().toISOString() }] }
        : w
      ))
    }
  ...
  const handleRemoveAsset = async (wlId: string, symbol: string) => {
    try {
      ...
      setWatchlists(p => p.map(w => w._id === wlId
        ? { ...w, assets: w.assets.filter(a => a.symbol !== symbol) }
        : w
      ))
    }
  }
  ...
  return (
    <div className="p-5 sm:p-6 max-w-[1400px] mx-auto page-enter animate-fade-in">
      <div className="flex items-start justify-between gap-4 mb-5">
        <div>
          <h1 className="text-xl font-semibold text-[var(--text-1)]">Watchlists</h1>
          <p className="text-xs text-[var(--text-3)] mt-0.5">Track your favourite financial assets</p>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          <AnimatePresence>
            {watchlists.map((wl, i) => (
              ...
              >
                {/* Level 2 card */}
                <div className="bg-white dark:bg-[#131D2E] border border-slate-200/80 dark:border-[var(--border)] rounded-xl p-5 hover:border-slate-300 dark:hover:bg-[var(--bg-raised)] transition-colors group flex flex-col h-full">
                  <div className="flex items-start justify-between mb-3">
                    <div className="flex-1 min-w-0">
                      <h3 className="font-bold text-[var(--text-1)] text-sm truncate">{wl.name}</h3>
                      {wl.description && <p className="text-xs text-[var(--text-2)] mt-0.5 line-clamp-1">{wl.description}</p>}
                    </div>
                  ...
                  <div className="flex flex-wrap gap-1.5 mb-3 flex-1 min-h-[28px]">
                    ...
                    {wl.assets.length > 6 && <span className="text-xs text-[var(--text-3)] px-1 py-0.5">+{wl.assets.length - 6}</span>}
                    {wl.assets.length === 0 && <span className="text-xs text-[var(--text-3)] italic">No assets yet</span>}
                  </div>
                  ...
                  <div className="flex items-center justify-between pt-2 border-t border-[var(--border)] mt-auto">
                    <div className="flex items-center gap-1.5">
                      ...
                      <Badge variant={wl.isPublic ? 'blue' : 'slate'}>
                        {wl.isPublic
                          ? <><Eye className="h-2.5 w-2.5 mr-1 inline" />Public</>
                          : <><EyeOff className="h-2.5 w-2.5 mr-1 inline" />Private</>
                        }
                      </Badge>
                    </div>
                    {wl.tags.length > 0 && (
                      <div className="flex gap-1">
                        {wl.tags.slice(0, 2).map(t => (
                          <span key={t} className="text-[10px] text-[var(--text-3)]">#{t}</span>
                        ))}
                      </div>
                    )}
                  </div>
      ...
      {createOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <motion.div initial={{ opacity: 0, scale: 0.95, y: 12 }} animate={{ opacity: 1, scale: 1, y: 0 }} className={modalCls}>
            <h2 className="text-base font-bold text-[var(--text-1)] mb-5">Create Watchlist</h2>
            <form onSubmit={cf.handleSubmit(handleCreate)} className="space-y-4">
              ...
              <label className="flex items-center gap-2 cursor-pointer">
                ...
                <span className="text-sm text-[var(--text-2)]">Make public</span>
              </label>
      ...
      {addOpen && selectedWL && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <motion.div initial={{ opacity: 0, scale: 0.95, y: 12 }} animate={{ opacity: 1, scale: 1, y: 0 }}
            className="relative w-full max-w-sm bg-white dark:bg-[#1A2540] border border-slate-200 dark:border-[var(--border-md)] rounded-2xl p-6 shadow-2xl z-10"
          >
            <h2 className="text-base font-bold text-[var(--text-1)] mb-1">Add Asset</h2>
            <p className="text-xs text-[var(--text-2)] mb-5">Add a symbol to "{selectedWL.name}"</p>
