...
export function Admin() {
  ...
  const loadMetrics = async () => {
    ...
    try {
      ...
      setMetrics({ period: '24h', metrics: { totalRequests: 1284, cacheHits: 891, apiCalls: 393, errors: 7, avgDuration: '142ms' } })
    ...
    <div className="p-5 sm:p-6 max-w-[1200px] mx-auto page-enter animate-fade-in">
      <div className="flex items-start justify-between gap-4 mb-5">
        <div>
          ...
          <p className="text-xs text-slate-500 mt-0.5">Cache management, metrics &amp; error logs</p>
        </div>
        <Button variant="ghost" size="sm" onClick={loadMetrics} loading={loading} icon={<RefreshCw className="h-3.5 w-3.5" />}>Refresh</Button>
      </div>
        ...
        <div className="space-y-4">
          ...
          <div className="bg-white dark:bg-[#131D2E] border border-slate-200 dark:border-slate-700/50 rounded-xl p-5">
            <div className="flex items-center gap-2 mb-4">
              ...
              <h3 className="text-sm font-semibold text-slate-900 dark:text-white">Metrics (24h)</h3>
            </div>
            {loading ? (
              <div className="space-y-2">{[...Array(5)].map((_, i) => <Skeleton key={i} className="h-9 rounded-xl" />)}</div>
            ) : metrics ? (
              <div className="space-y-2">
                <p className="text-[10px] text-slate-400 dark:text-slate-600 uppercase tracking-wide mb-3">Period: {metrics.period}</p>
                {Object.entries(metrics.metrics ?? {}).slice(0, 8).map(([k, v]) => (
                  <div key={k} className="flex items-center justify-between p-2.5 rounded-xl bg-slate-50 dark:bg-slate-800/40">
                    <span className="text-xs text-slate-600 dark:text-slate-400 capitalize">{k.replace(/_/g, ' ')}</span>
